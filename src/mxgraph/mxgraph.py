...
class MxCell(MxBase):
    ...
    def parent(self):
        ...
        if self._parent_id is None:
            return None
        #return self.cell_store[self._parent_id]      ## BUG
        return self._parent_id   # mxgraph.mxgraph.MxCell

        #print(type(self._parent_id))
        #if self._parent_id in self.cell_store:
    ...
    def to_xml(self):
        ...
        if self.parent is not None:                              #### BUG
            if type(self.parent) is str: 
              cell_xml.set('parent', self._parent_id)
            else:
              cell_xml.set('parent', self.parent.cell_id)
            #cell_xml.set('parent', self._parent_id)
        #if self._parent_id is not None:                          ### BUG
        #    cell_xml.set('parent', self._parent_id)
