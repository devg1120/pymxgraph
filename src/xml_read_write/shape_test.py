...
def modify_file2(filename, filename2):
    ...
    g.insert_vertex(parent="G0avHX12ZUL-R_R_pCqJ-4", value = "task_q" , x=340 , y=40, width=20, height=20, style=style4, relative=False )
    ...
    shape_data4 = ""

    #decoded = base64.decodebytes(shape_data.encode('utf-8'))
    decoded = base64.b64decode(shape_data.encode('utf-8'))
    compress = zlib.decompress(decoded, -zlib.MAX_WBITS)
    data2 = urllib.parse.unquote(compress)
    print("shape_data----------------------")
    print(shape_data)
    print("base64 decode----------------------")
    print(decoded)
    ...
    ##############################################################
    print("shape_encode----------------------")
    data2_ = urllib.parse.quote(data2).encode('utf-8')
    print("urlencode quote-------------------")

    print(urllib.parse.unquote(data2_))
    print(urllib.parse.unquote(compress))




    #data3 = zlib.compress(data2_)
    #data3 = zlib.compress(data2_, level = 1 )

    #data3 = pako_deflate_raw(data2_)
    data3 = pako_deflate_raw(compress)
    ...
    #data3 = zlib.compress(data2_.encode('utf-8'))
    # data3 = zlib.compress(data2_ )
    #data3 = pako_deflate_raw(data2_.encode('utf-8'))
    data3 = pako_deflate_raw(data.decode('utf-8'))
    data4 = pako_deflate_raw(compress.decode('utf-8'))
    print("--------------------------------------------")
    print(data3)
    print("--------------------------------------------")
    print(data4)
    ...
    #data3 = urllib.parse.unquote(data3)
    encoded = base64.b64encode(data3)
    #encoded = base64.b64encode(data4)
    ...
    g.insert_vertex(parent="G0avHX12ZUL-R_R_pCqJ-4", value = "task_y" , x=840 , y=240, width=100, height=100, style=style8, relative=False )
...
def shape_encode( data ):
    ...
    #data3 = zlib.compress(data2_)
    #data3 = zlib.compress(data2_, level = 1 )
    #cobj = zlib.compressobj(level = -1, method = 8 )
    #data3 = cobj.compress(data2_)

    data3 = pako_deflate_raw(data2_)
    #data3 = pako_deflate(data2_)
